...
try:
    ort.preload_dlls()  # pip nvidia-cu12 paketlaridan CUDA/cuDNN (ort>=1.21)
except Exception:
    pass

# GPU bo'lsa CUDA, aks holda CPU
_PROVIDERS = (
...
class BaseRecognition:
    ...
    def run(self):
        ...
        if self.session is None:
            self.session = ort.InferenceSession(self.model_path, providers=_PROVIDERS)
...
if __name__ == "__main__":
    ...
    # GPU bo'lsa fp16 modelni ishlatamiz (4GB VRAM ga sig'adi va ~28x tezroq)
    model_path = conf.model_path
    if _PROVIDERS[0] == "CUDAExecutionProvider" and os.path.exists("mvit32-2-fp16.onnx"):
        model_path = "mvit32-2-fp16.onnx"
    logger.info(f"Provayder: {_PROVIDERS[0]} | model: {model_path}")
    runner = Runner(model_path, conf, args.mp, args.verbose, args.length)
    runner.run()
